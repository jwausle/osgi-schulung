...
public class HelloWorld extends Thread implements ManagedService {
    ...
    public void updated(Dictionary<String, ?> properties)
        ...
        waitTime = Optional.ofNullable(properties)
            .map(props -> {
                Arrays.asList(props.keys()).stream()
                    .map(key -> System.out.printf("%s %s%n", key, properties.get(key)));
                return props;
            })
            .map(props -> props.get("waitTime"))
            .map(string -> Long.valueOf(string.toString())).orElse(5000L);
        if (!isAlive()) {
            start();
